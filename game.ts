enum Cell {
  ...
  // 盤面（8x8の2次元配列）を初期化
  let board: Cell[][] = Array.from({ length: BOARD_SIZE }, () =>
  ...
  // 現在の手番（初手は黒＝プレイヤー）
  let currentPlayer: Cell = Cell.Black;
  ...
  function renderBoard() {
    const boardTable = document.getElementById("board") as HTMLTableElement;
  ...
  function updateMessage(message?: string) {
    const msgDiv = document.getElementById("message") as HTMLElement;
    if (message) {
  ...
  function getFlippableStones(x: number, y: number, color: Cell): { x: number, y: number }[] {
    const opponent = (color === Cell.Black) ? Cell.White : Cell.Black;
    const stonesToFlip: { x: number, y: number }[] = [];
  
    if (board[y][x] !== Cell.Empty) return [];
  
    // 各方向について探索
    for (const dir of directions) {
      let nx = x + dir.dx;
      let ny = y + dir.dy;
      const stonesInDirection: { x: number, y: number }[] = [];
      // まずは隣接セルが相手の石でなければならない
      if (!isInside(nx, ny) || board[ny][nx] !== opponent) continue;
      // 相手の石が続く間探索
  ...
  // 指定の位置に色を置けるかチェック
  function isValidMove(x: number, y: number, color: Cell): boolean {
    return getFlippableStones(x, y, color).length > 0;
  }
  
  // 合法手の一覧を返す
  function getValidMoves(color: Cell): { x: number, y: number }[] {
  ...
  // 指定セルに石を置き、必要な石を反転する
  function makeMove(x: number, y: number, color: Cell): boolean {
  ...
  function onCellClick(event: MouseEvent) {
    if (currentPlayer !== Cell.Black) return; // プレイヤーの手番でない場合は無視
  ...
  function gameOver() {
    ...
    for (let y = 0; y < BOARD_SIZE; y++) {
      for (let x = 0; x < BOARD_SIZE; x++) {
        if (board[y][x] === Cell.Black) blackCount++;
        else if (board[y][x] === Cell.White) whiteCount++;
      }
    }
    let result = `ゲーム終了！ 黒: ${blackCount}、白: ${whiteCount}。`;
    if (blackCount > whiteCount) result += "あなたの勝ちです！";
    else if (whiteCount > blackCount) result += "コンピュータの勝ちです！";
    else result += "引き分けです！";
